...
const userCrl = {
  ...
  getUserById: async (req, res) => {
    try {
      ...
      const userMe = await User.findById({ _id: meId });
      if(userMe.new_user=== true) {
        const userList= await User.find({}, "_id username phoneNumber email address")
        let userFilterMe= userList?.filter(item=> item._id != meId)
        let listSetupConversation= []
        userFilterMe.map(item=> listSetupConversation.push({
          member: [userMe._id, item._id],
          createdBy: userMe._id,
          type: 0 
        }))
        // console.log(userMe)
        const setup = await Conversation.insertMany(listSetupConversation)
        .then(data => Promise.all(data.map(item => {
          return Conversation.findByIdAndUpdate(item._id, { id_conversation: item._id }, { new: true });
        })))
        .catch(error => {
          // Xử lý lỗi nếu có
          console.error('Đã xảy ra lỗi trong quá trình insertMany hoặc update:', error);
          throw error; // Ném lỗi để báo cho phần xử lý khác biết rằng đã có lỗi xảy ra
        });
        const { password, updatedAt, ...other } = userMe._doc;
        ...
        return res.status(200).json({...other, ok: true});
  ...
  statusRequest: expressAsyncHandler(async (req, res)=> {
    try {
      ...
      if (thangNhan._doc.friendsQueue.includes(idcuaMinh)) {
        ...
        return res.status(200).json({msg: "", request: false, duplicate: false})
      }
      
    } catch (error) {
      return res.status(500).json({msg: "", ok: false})
  ...
  setupConversation: expressAsyncHandler(async (req, res)=> {
    try {
      const user= await User.findById({_id: req.user.id})
