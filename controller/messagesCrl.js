...
const messagesCrl = {
   postMessage: async (req, res) => {
    ...
    recallMessage: async (req, res)=> {
        try {   
            const recall= await Message.findOneAndUpdate({key: req.params.keyId}, {type_message: "text", message: req.body.message, status_message: 2})
            res.status(200).json({
                data: recall
            })
        } catch (error) {
    ...
    removeMessage: async (req, res)=> {
        try {
            const remove= await Message.findOneAndUpdate({key: req.params.keyId}, {type_message: "text", message: req.body.message, status_message: 3})
            res.status(200).json({
                data: remove
            })
        } catch (error) {
